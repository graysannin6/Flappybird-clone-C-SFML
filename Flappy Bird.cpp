// Flappy Uffo.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
void draw() {
    ...
    bird->draw();
    if (not gameOvered) {
        sf::Text scoreText("Score: " + to_string(points), *font);
    ...
    if (gameOvered)
    {
        sf::Text scoreText("Your Score is: " + to_string(points), *font);
        scoreText.setPosition(window->getSize().x / 2 - scoreText.getLocalBounds().width / 2, 300);
        window->draw(scoreText);
    }
